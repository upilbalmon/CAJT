...
-- ============================================
-- FUNGSI PENCEGAHAN GUI GANDA (DARI LOADER.LUA)
-- ============================================
local function destroyExistingGUI()
    local existingGUI = playerGui:FindFirstChild("DrawHeroLoopGUI")
    if existingGUI then
        existingGUI:Destroy()
        return true
    end
...
StatusLabel.Position = UDim2.new(0, 5, 1, -120)
...
DrawLabel.Position = UDim2.new(0, 5, 1, -100)
DrawLabel.Text = "Draw:"
...
DrawButtonFrame.Position = UDim2.new(0.18, 0, 1, -100)
...
AutoDetectButton.Position = UDim2.new(0.7, 0, 1, -100)
...
IdLabel.Position = UDim2.new(0, 5, 1, -78)
IdLabel.Text = "ID:"
...
-- ============================================
-- ID BOX - DENGAN LOGICA DARI INFINITY.LUA
-- ============================================
local IdBox = Instance.new("TextBox")
IdBox.Size = UDim2.new(0.35, -10, 0, 18)
IdBox.Position = UDim2.new(0.15, 0, 1, -78)
...
IdBox.TextSize = 11
IdBox.ClearTextOnFocus = false  -- <-- KUNCI: Mencegah teks terhapus saat diklik
...
DetectNowButton.Position = UDim2.new(0.53, 0, 1, -78)
...
DelayLabel.Position = UDim2.new(0, 5, 1, -56)
...
DelayBox.Position = UDim2.new(0.15, 0, 1, -56)
...
StartStopButton.Position = UDim2.new(0.38, 0, 1, -56)
...
IdDetectedLabel.Position = UDim2.new(0, 5, 1, -36)
...
DrawCountDisplay.Position = UDim2.new(0, 5, 1, -18)
...
--// Unhook (METODE DARI AUTO COIN V3)
...
local function DrawHeroFunction()
    ...
    local success, result = pcall(function()
        return DrawHeroEvent:InvokeServer(heroId, drawCount)
    end)
...
StartStopButton.MouseButton1Click:Connect(function()
    ...
    if running then
        ...
        coroutineLoop()
        UpdateStatus("▶️ Running... ID: " .. heroId .. " | " .. drawCount .. "x", Color3.fromRGB(0, 255, 0))
    else
        StartStopButton.Text = "Start"
        StartStopButton.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
...
--// CLOSE BUTTON - METODE DARI AUTO COIN V3
CloseButton.MouseButton1Click:Connect(function()
    -- Metode sederhana dari AUTO COIN V3
    hookEnabled = false
    running = false
    coroutineLoop = nil
    MainFrame:Destroy()
...
IdBox.FocusLost:Connect(function(enterPressed)
    ...
    if newId and newId > 0 then
        ...
        -- Jika input tidak valid, kembalikan ke heroId terakhir yang valid
        IdBox.Text = tostring(heroId)
        UpdateStatus("⚠️ ID tidak valid, dikembalikan ke: " .. heroId, Color3.fromRGB(255, 165, 0))
    end
end)
